...
static NSInteger const apiResultsLimit = 90;
...
- (void)handleRightSwipe:(UIGestureRecognizer*)sender {
    if (sender.state == UIGestureRecognizerStateEnded && self.apiPage != 0) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"nextSegue"]) {
        ...
        collectionVC.apiPage = self.apiPage + 1;
...
- (NSInteger)offset {
    return _apiPage * apiResultsLimit;
}
